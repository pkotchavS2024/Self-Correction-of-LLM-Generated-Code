...
class Config:
    ...
    num_epochs_stage_one: int = 20
    num_epochs_stage_two: int = 20
...
class SCoReTrainer:
    ...
    def reward_function_code(self, code: str, test: str, stage: str = None, attempt: str = None, timeout: int = 2) -> Tuple[float, float]:
        """
        ...
        if 'input(' in code or 'raw_input(' in code:
            logger.warning("Code contains input request - assigning zero reward")
            return 0.0, 0.0

        if 'while True' in code or 'for _ in range(99999' in code:
            logger.warning("Code contains potential infinite loop - assigning zero reward")
            return 0.0, 0.0
        ...
        for pattern in dangerous_patterns:
            if pattern in code:
                logger.warning(f"Code contains dangerous pattern '{pattern}' - assigning zero reward")
                return 0.0, 0.0
        ...
        try:
            ...
            def execute_with_timeout(code_to_run: str, globals_dict: dict) -> None:
                try:
                    exec(code_to_run, globals_dict)
                except Exception:
                    # Silently catch exceptions - they'll be handled by the timeout check
                    pass
            ...
            try:
                ...
                # thread= threading.Thread(target=execute_with_timeout, args=(code, exec_globals))
                # thread.daemon = True  # Set as daemon thread so it dies with the main thread
                # thread.start()
                # thread.join(timeout=timeout)  # Wait for timeout seconds
                logger.info("✓ Code execution successful")

                # if thread.is_alive():
                #     logger.warning(f"Code execution timed out after {timeout} seconds")
                #     trace_info["generated_code"]["execution_result"] = "timeout"
                #     return 0.0, 0.0
                ...
                for i, test_case in enumerate(test_cases, 1):
                    ...
                    try:
                        ...
                        exec(cleaned_test, exec_globals)
                        test_result["passed"] = True
                        trace_info["test_cases"]["passed"] += 1
                        logger.info(f"Test {i}: ✓ {cleaned_test}")

                        # test_thread = threading.Thread(target=execute_with_timeout, args=(cleaned_test, exec_globals))
                        # test_thread.daemon = True
                        # test_thread.start()
                        # test_thread.join(timeout=timeout)

                        # if test_thread.is_alive():
                        #     logger.warning(f"Test case {i} timed out after {timeout} seconds")
                        ...
                        logger.info(f"Test {i}: × Failed assertion: {cleaned_test}")
                    except Exception as e:
                        all_tests_passed = False
